...
const Home = () => {
  ...
  return (
    <section
      className="bg-slate-800 w-5/5 flex pt-35 sm:pt-8 h-screen w-full sm:flex-col"
      id="home"
    >
      <motion.div
        ...
        className="w-5/5 gap-y-2 sm:mt-20 flex flex-col pl-20 sm:pl-0 flex flex-col  justify-center items-center sm:w-4/5 sm:m-auto"
      >
      ...
      <motion.div
        ...
        className=" w-4/5 sm:h-[300px] md:h-[400px] gap-y-5 flex flex-col  flex justify-center items-center mt-8  sm:mt-3 sm:m-auto  sm:rounded-full sm:hidden"
      >
        <img
          whileHover={{ scale: 1.2 }}
          src={img}
          alt="profile image"
          className="w-86 h-80 sm:w-60 h-80 sm:ml-auto m-auto sm:m-auto sm:mt-0 md:mt-0 sm:rounded-full"
        />
      </motion.div>
    </section>
  );
